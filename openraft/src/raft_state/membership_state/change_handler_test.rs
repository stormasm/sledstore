...
fn test_apply_not_committed() -> anyhow::Result<()> {
    ...
    let res = new()
        .change_handler()
        .apply(ChangeMembers::AddVoterIds(btreeset! {1}), false);
...
fn test_apply_empty_voters() -> anyhow::Result<()> {
    ...
    let res = new()
        .change_handler()
        .apply(ChangeMembers::RemoveVoters(btreeset! {1}), false);
...
fn test_apply_learner_not_found() -> anyhow::Result<()> {
    ...
    let res = new()
        .change_handler()
        .apply(ChangeMembers::AddVoterIds(btreeset! {2}), false);
...
fn test_apply_retain_learner() -> anyhow::Result<()> {
    ...
    let res = new()
        .change_handler()
        .apply(ChangeMembers::RemoveVoters(btreeset! {1,2}), false);
    assert_eq!(
        Ok(Membership::new(
            vec![btreeset! {3,4,5}],
            btreemap! {3=>(),4=>(),5=>()}
    ...
    let res = new()
        .change_handler()
        .apply(ChangeMembers::RemoveVoters(btreeset! {1,2}), true);
    assert_eq!(
        Ok(Membership::new(
            vec![btreeset! {3,4,5}],
            btreemap! {1=>(),2=>(),3=>(),4=>(),5=>()}
